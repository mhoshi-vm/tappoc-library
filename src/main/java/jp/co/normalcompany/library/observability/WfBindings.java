...
public class WfBindings implements BindingsPropertiesProcessor {
    ...
    public void process(Environment environment, Bindings bindings, Map<String, Object> map) {
        ...
        if (wfBindings.isEmpty()) {
            ...
            map.put("management.endpoint.wavefront.enabled", "true");
            map.put("management.wavefront.metrics.export.enabled", "true");
            map.put("management.tracing.enabled", "true");
            map.put("management.tracing.sampling.probability", "1.0");
        }
        bindings.filterBindings(TYPE).forEach(binding -> {
            var mapper = new WfBindings.BindingPropertiesMapper(binding.getSecret(), map);
